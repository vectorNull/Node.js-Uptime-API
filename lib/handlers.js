...
handlers._checks.post = (data, callback) => {
	...
	var successCodes =
		...
		data.payload.successCodes.trim().length > 0
										...
										_data.update(
											...
											(err) => {
												if (!err) {
													...
													callback(500, {
														Error:
															'Could not update the user with new check',
													});
												}
											},
										);
			...
			if (!err && checkData) {
				let token =
					typeof data.headers.token == 'string'
						? data.headers.token
						: false;
				// Verify given token is valid and belongs to user
